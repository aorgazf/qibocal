...
@dataclass
class InterleavedRBParameters(Parameters):
    """Interleaved RB runcard inputs."""
...
@dataclass
class InterleavedRBResults(Results):
    """Interleaved RB outputs."""
...
class InterleavedRBData(DataUnits):
...
def _acquisition(
    params: InterleavedRBParameters, platform: AbstractPlatform, qubits: Qubits
) -> InterleavedRBData:
    r"""
    ...
    for qubit in qubits.values():
        ...
        for sequence, circuit in zip(sequences.values(), circuits.values()):
            # Temporary fix, all sequences will be executed as 'zhinst' in the future
            # if platform.name == "zhinst":
...
def _fit(data: InterleavedRBData) -> InterleavedRBResults:
    """Post-processing for Interleaved RB."""
    ...
    return InterleavedRBResults(
        fidelities, fidelities_primitive, average_errors_gate, fitted_parameters
    )


def _plot(data: InterleavedRBData, fit: InterleavedRBResults, qubit):
    """Plotting function for Interleaved RB."""
    return utils.plot(data, fit, qubit)


interleaved_rb = Routine(_acquisition, _fit, _plot)
"""Interleaved RB Routine object."""
