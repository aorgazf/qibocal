...
def cryoscope(folder, routine, qubit, format):
    ...
    # phi_unwrap --> unwrapped phase along duration axis (axis = 1)
    # phi_unwrap_amplitude --> unwrapped phase along amplitude axis (axis = 0)
    ...
    figs["detuning_vs_amplitude"] = go.Figure()  # 5 for the stable detuning frequency as a function of amplitude

    detuning_median = np.nanmedian(detuning, axis=1)  # Stable detuning after a long pulse
    ...
    coeff_fit = np.polyfit(
        ...
        detuning_median_amplitude[amplitude_unique < 0.05],
        2,
    )
    ...
    for i, amp in enumerate(amplitude_unique):
        ...
        awg_reconstructed_pulse_tests["FitMedianAmplitude_InstantAmplitude"] = np.interp(
        ...
        #NOT working above 0.05 amplitude
    ...
    for i, amp in enumerate(amplitude_unique):
        ...
        for j in range(len(awg_reconstructed_pulse_tests)):
            step["args"][1][len(awg_reconstructed_pulse_tests)*i+j] = True  # Toggle i'th trace to "visible"
        steps.append(step)
    sliders = [
        dict(steps=steps, currentvalue={"prefix": "Amplitude (a.u.) = "})
    ]
    ...
    return figs["reconstructed_pulse"]
