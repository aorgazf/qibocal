...
# load the offset instead of the current and call the function frequency_flux_offset_msr_phase


def frequency_flux_offset_msr_phase__matrix(folder, routine, qubit, format):
    fluxes = []
    for i in range(25):  # FIXME: 25 is hardcoded
        file = f"{folder}/data/{routine}/data_q{qubit}_f{i}.csv"
        if os.path.exists(file):
    ...
    for j in fluxes:
        ...
        fig.add_trace(
            go.Heatmap(
                x=data.get_values("frequency", "GHz"),
                y=data.get_values("offset", "dimensionless"),
                z=data.get_values("MSR", "V"),
                showscale=showscale,
            ),
            row=1,
...
def frequency_flux_offset_msr_phase(folder, routine, qubit, format):
    ...
    fig.update_layout(
        ...
        yaxis_title="Offset (dimensionless)",
        xaxis2_title="Frequency (GHz)",
        yaxis2_title="Offset (dimensionless)",
    )
    return fig


def frequency_gain_msr_phase(folder, routine, qubit, format):
    data = DataUnits.load_data(folder, routine, format, f"data_q{qubit}")
    fig = make_subplots(
        rows=1,
        cols=2,
...
# Function to plot the frequency vs amplitude (dimensionless) of the ro_pulse
def frequency_amplitude__msr_phase(folder, routine, qubit, format):
    data = DataUnits.load_data(folder, routine, format, f"data_q{qubit}")
    fig = make_subplots(
        rows=1,
        cols=2,
