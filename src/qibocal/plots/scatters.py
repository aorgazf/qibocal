...
def exc_gnd(folder, routine, qubit, format):
    ...
    print(f"iq_mean_exc {qubit}   {complex(i_mean_exc, q_mean_exc)}")
    mod_iq_exc = abs(iq_mean_exc) * 1e6
    print(f"mod_iq_exc {qubit}   {abs(iq_mean_exc) * 1e6}")
    ...
    print(f"iq_mean_gnd {qubit}   {complex(i_mean_gnd, q_mean_gnd)}")
    mod_iq_gnd = abs(iq_mean_gnd) * 1e6
    print(f"mod_iq_gnd {qubit}   {abs(iq_mean_gnd) * 1e6}")
...
def exc_gnd_RX90(folder, routine, qubit, format):
    ...
    try:
        data_RX90 = Dataset.load_data(folder, routine, format, f"data_RX90_q{qubit}")
    except:
        data_RX90 = Dataset(quantities={"iteration": "dimensionless"})

    fig.add_trace(
        go.Scatter(
            x=data_RX90.get_values("i", "V"),
            y=data_RX90.get_values("q", "V"),
            name="RX90 state",
            mode="markers",
