    ...
    if qubit == fluxline:
        ...
        try:
            popt, pcov = curve_fit(
                ...
                curr,
                freq_norm,
                sigma=(freq_error_arr - freq_min1) / (freq_max1 - freq_min1),
            )
            ...
            curr_max_err = np.abs(popt[2] / (2 * np.pi)) * np.sqrt(
                np.abs(pcov[3, 3])
            ) + np.abs((n * np.pi - popt[3]) / (2 * np.pi)) * np.sqrt(
                np.abs(pcov[2, 2])
            )
            freq_max = cos(curr_max, *popt)
            freq_max_err = (
                np.sqrt(np.abs(pcov[1, 1])) + np.sqrt(np.abs(pcov[0, 0]))
            ) * (freq_max1 - freq_min1)
            freq_offset = cos(0, *popt)
            freq_offset_err = (
                np.sqrt(np.abs(pcov[0, 0])) * (freq_max1 - freq_min1)
                + np.abs(np.cos(popt[3]))
                * np.sqrt(np.abs(pcov[1, 1]))
                * (freq_max1 - freq_min1)
                + np.abs(np.sin(popt[3]) * popt[1]) * np.sqrt(np.abs(pcov[3, 3]))
            )
            # freq_offset=freq_max-freq_zero
            # freq_offset_err=freq_max_err+freq_zero_err
            C_ii = (freq_max - freq_offset) / curr_max
            C_ii_err = (
                freq_max_err / np.abs(curr_max)
                + freq_offset_err / np.abs(curr_max)
                + np.abs((freq_max - freq_offset) / curr_max**2) * curr_max_err
            )
        except:
            log.warning("The fitting was not succesful")
            return data_fit
