...
@plot("CZ phase", plots.duration_rectangular)
def duration_rectangular(
    platform: AbstractPlatform,
    qubit: int,
    duration_start,
    duration_end,
    ...
    r"""
    This experiment is used to find the duration of a Rectangular() pulse to have
    a 180 CZphase.
    ...
    find the detuning acquired by the flux pulse. The phase of both ON and OFF
    sequences are measured and the difference between the two is used to find
    the duration to perform a CZ gate (180 phase difference).

    Args:
        platform: The platform to run the experiment on.
        qubit: The qubit to perform the experiment on.
        duration_start: The starting duration of the flux pulse in ns.
        duration_end: The ending duration of the flux pulse in ns.
        duration_step: The step size of the flux pulse in ns.
        detuning_start: The starting phase of the second RX90 pulse in degree to
            the target qubit.
        detuning_end: The ending phase of the second RX90 pulse in degree to the
            target qubit.
    ...
    # Find the control and target qubits
    # FIXME: qubit should be a list of qubits
    qubits = [2, 3]
    ...
    for i, q_target in enumerate(qubit_target):
        ...
        flux_pulse = FluxPulse(
            **platform.native_gates["two_qubit"][f"{q_target}-{qubit_control[i]}"][
                "CZ_rectangular"
            ]
        )
        flux_pulse.start = initial_RX90_pulse.se_finish + 8
